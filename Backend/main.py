...
def get_stvar(stvar_to_get):
    try:
        with orm.db_session:
            ...
            result = orm.select(
                x for x in Stvar if x.stvar == stvar_to_get)[:][0]
...
def patch_stvar(stvar_to_patch, cijena_to_patch):
    try:
        with orm.db_session:
            to_update = orm.select(
                x for x in Stvar if x.stvar == stvar_to_patch)[:][0]
...
def delete_stvar(stvar_to_delete):
    try:
        with orm.db_session:
            to_delete = orm.select(
                x for x in Stvar if x.stvar == stvar_to_delete)[:][0]
            to_delete.delete()
            response = {"response": "Success"}
            return response
    except Exception as e:
